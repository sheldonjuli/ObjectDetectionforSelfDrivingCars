...
switch whatdata
    ...
    case {'superpixels'}
        ...
        data.spim = spim;
   case {'load-detector-result'}
        files = dir(fullfile(DETECTOR_RESULT_DIR, sprintf('%s_*.mat', imname)));
        if isempty(files)
            fprintf('file doesn''t exist!\n');
        else
            data.bs_car = load(fullfile(DETECTOR_RESULT_DIR, files(1).name));
            data.bs_cyclist = load(fullfile(DETECTOR_RESULT_DIR, files(2).name));
            data.bs_person = load(fullfile(DETECTOR_RESULT_DIR, files(3).name));
            data.ds_car = load(fullfile(DETECTOR_RESULT_DIR, files(4).name));
            data.ds_cyclist = load(fullfile(DETECTOR_RESULT_DIR, files(5).name));
            data.ds_person = load(fullfile(DETECTOR_RESULT_DIR, files(6).name));
        end;
   case {'load-depth-result'}
        files = dir(fullfile(DEPTH_RESULT_DIR, sprintf('%s_depth*.mat', imname)));
        if isempty(files)
            fprintf('file doesn''t exist!\n');
        else
            data = load(fullfile(DEPTH_RESULT_DIR, files(1).name));
        end;
   case {'load-location-result'}
        files = dir(fullfile(LOCATION_RESULT_DIR, sprintf('%s_*_location*.mat', imname)));
        if isempty(files)
            fprintf('file doesn''t exist!\n');
        else
            data.lo_car = load(fullfile(LOCATION_RESULT_DIR, files(1).name));
            data.lo_cyclist = load(fullfile(LOCATION_RESULT_DIR, files(2).name));
            data.lo_person = load(fullfile(LOCATION_RESULT_DIR, files(3).name));
        end;
    otherwise 
